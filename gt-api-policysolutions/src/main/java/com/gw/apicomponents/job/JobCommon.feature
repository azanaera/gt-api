Feature: As a user, I want to modify a job
  ...
  @id=ValidateCostCount
  Scenario: Verify cost count
    * def requiredArguments = ['jobId', 'expectedInstalledCostCount', 'expectedVisualizedCostCount', 'visualized']
    * def getCostsUrl = jobUrl + "/" + __arg.scenarioArgs.jobId + '/costs'
    * def getExpectedCostsCount =
      """
        function(){
            if(__arg.scenarioArgs.visualized){
                 return __arg.scenarioArgs.expectedVisualizedCostCount;
           } else {
                 return __arg.scenarioArgs.expectedInstalledCostCount;
           }
        }
      """
    Given url getCostsUrl
    When method GET
    Then status 200
    And match response.count == getExpectedCostsCount()

  @id=ValidateTotalCostAndPremium
  Scenario: Verify Total Cost and Total Premium
    * def requiredArguments = ['jobId', 'expectedInstalledTotalCost', 'expectedInstalledTotalPremium', 'expectedVisualizedTotalCost', 'expectedVisualizedTotalPremium', 'expectedCurrency', 'visualized']
    * def jobPremiumUrl = jobUrl +  "/" + __arg.scenarioArgs.jobId
    * def getExpectedTotalCost =
      """
        function(){
            if(__arg.scenarioArgs.visualized){
                 return __arg.scenarioArgs.expectedVisualizedTotalCost;
           } else {
                 return __arg.scenarioArgs.expectedInstalledTotalCost;
           }
        }
      """
    * def getExpectedTotalPremium =
      """
        function(){
            if(__arg.scenarioArgs.visualized){
                 return __arg.scenarioArgs.expectedVisualizedTotalPremium;
           } else {
                 return __arg.scenarioArgs.expectedInstalledTotalPremium;
           }
        }
      """
    Given url jobPremiumUrl
    When method GET
    Then status 200
    And match response.data.attributes.totalCost.amount == getExpectedTotalCost()
    And match response.data.attributes.totalCost.currency == __arg.scenarioArgs.expectedCurrency
    And match response.data.attributes.totalPremium.amount == getExpectedTotalPremium()
    And match response.data.attributes.totalPremium.currency == __arg.scenarioArgs.expectedCurrency