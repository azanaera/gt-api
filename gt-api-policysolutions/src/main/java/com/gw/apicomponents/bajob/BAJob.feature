Feature: As a user, I want to modify a job
  ...
  Background:
    * def sharedPath = 'classpath:com/gw/apicomponents/bajob/'
  ...
  @id=SyncJurisdictionCoverages
  Scenario: Sync coverages of the specified jurisdiction
    * def requiredArguments =  ['jobId','jurisdictionId']
    * def emptyRequestTemplate = emptyJsonPath + 'emptyRequest.json'
    * def syncJurisdictionCovUrl = jobUrl +  "/" + __arg.scenarioArgs.jobId + '/lines/BusinessAutoLine/ba-jurisdictions/' + __arg.scenarioArgs.jurisdictionId  + '/sync-coverages'
    Given url syncJurisdictionCovUrl
    And request read(emptyRequestTemplate)
    When method POST
    Then status 200
