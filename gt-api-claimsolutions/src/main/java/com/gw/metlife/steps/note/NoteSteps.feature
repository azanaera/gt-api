Feature: Note
  ...
  Scenario: I can add a note related to the service request with the following
    * def parameters = ['topic', 'body']
    * def unrestrictedUsername = claimUtils.getUnrestrictedUser()
    When step('NoteActions.CreateNoteForServiceRequest', {'scenarioArgs': {'username': unrestrictedUsername, 'claimId': __arg.cucumberDataCache.claimId}, 'templateArgs': {'topic': __arg.parameters.topic, 'body': __arg.parameters.body, 'serviceRequestId': __arg.cucumberDataCache.serviceRequestId}})
    Then step('NoteActions.GetNotesOnServiceRequest', {'scenarioArgs': {'username': unrestrictedUsername, 'claimId': __arg.cucumberDataCache.claimId, 'serviceRequestId': __arg.cucumberDataCache.serviceRequestId}})
