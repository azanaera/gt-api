Feature: Accounts
  ...
  Background:
    ...
    * def accountPath = 'classpath:com/gw/apicomponents/account/'
  ...
  @id=PatchAccountContact
  Scenario:  Patch an account contact
    * def requiredScenarioArguments = ['accountId','contactId']
    * def accountContactUrl = accountsUrl +  "/" + __arg.scenarioArgs.accountId + '/contacts/' + __arg.scenarioArgs.contactId
    * def requestPayload = readWithArgs(accountPath + 'patchContact.json', __arg.templateArgs)
    Given url accountContactUrl
    And request requestPayload
    When method PATCH
    Then status 200
